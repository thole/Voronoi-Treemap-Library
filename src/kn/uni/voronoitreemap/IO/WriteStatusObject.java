...
/**
 * Class to write the result of a Voronoi Treemap computation as an edge list,
 * including polygons
 * "nodeId;parentID;name;weight;hierarchyLevel;sitePosX;sitePosY;siteWeight;polygonPoints
 * x1,y2,x2,y2\n";
 * 
 * @author Arlind Nocaj
 *
...
public class WriteStatusObject implements StatusObject {
	...
	private void writeTreemap() {
		...
		for (VoroNode voroNode : treemap.getIdToNode().values()) {
			...
			if (voroNode.getParent() == null)
				continue;
			VoroNode parent = voroNode.getParent();
			Site site = voroNode.getSite();
			if (site == null) {
				System.out.println("site null: " + voroNode.getName() + "\t level: " + voroNode.getHeight()
			...
			builder.append(
					voroNode.getNodeID() + ";" + parent.getNodeID() + ";" + voroNode.name + ";" + voroNode.getWeight()
							+ ";" + voroNode.getHeight() + ";" + site.x + ";" + site.y + ";" + site.getWeight() + ";");
	...
	@Override
	synchronized public void finishedNode(int Node, int layer, int[] children, PolygonSimple[] polygons) {
